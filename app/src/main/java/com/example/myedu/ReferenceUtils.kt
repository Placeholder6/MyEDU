...
class ReferenceJsFetcher {
    ...
    suspend fun fetchResources(logger: (String) -> Unit): PdfResources = withContext(Dispatchers.IO) {
        try {
            ...
            // 2. Fetch Main JS to find References7 name
            ...
            // 3. Download the actual script content
...
/**
 * Stores the raw JS content of References7.js
 */
data class PdfResources(val scriptContent: String)
    ...
    suspend fun generatePdf(
        ...
        Handler(Looper.getMainLooper()).post {
            try {
                ...
                // Logic Extraction: 
                // We find the function that creates the PDF definition.
                // Pattern: const XX = (o,d,v) => { ... return { pageSize: "A4" ... } }
                // We look for "pageSize" and "pageOrientation" to verify it's the right function.
                val jsContent = resources.scriptContent
                
                // Regex to extract the generator function body.
                // Finds: const [name] = (arg1, arg2, arg3) => { ... content ... }
                // It stops when it hits the start of the Vue component definition "const ... = defineComponent({"
                ...
                if (match != null) {
                    // We replace the function name with 'generateDocDef' so we can call it easily
                    ...
                    // Fallback: if regex fails, try to just use the known start/end points based on your provided file
                    // Start: const at=(
                    // End: const nt={class:"card"}
                ...
                // Imports that the extracted function relies on:
                // 1. PdfStyle (tt in the file) -> We need to mock this or extract it too.
                // In your file: import { P as tt } from "./PdfStyle..."
                // The generator uses 'tt' at "styles: tt". We must define 'tt'.
                // We will define 'tt' as a dummy object for styles.
                
                // 2. Signed (et in the file) -> image. We can mock this image or fetch it.
                // In file: import { S as et } from "./Signed..."
                // We must define 'et'.
                ...
                <script>
                    ...
                    // 2. MOCKS for Imported Dependencies
                    // The extracted function uses 'tt' for styles and 'et' for signature image.
                    // We declare them globally.
                    ...
                    // Mock Signed Image (et). Should be base64.
                    // If you have the actual Signed.js url, we could fetch it, but empty/dummy is safer for now.
                    // This avoids 'variable not defined' errors.
                    ...
                    // 3. INJECTED GENERATOR
                    // This is the code we extracted from the file (const generateDocDef = ...)
                    $extractedFunction

                    // 4. Logic to prepare arguments (Translation & Course Calc)
                    ...
                    function prepareAndGenerate() {
                        try {
                            ...
                            // C. CALL EXTRACTED FUNCTION
                            // generateDocDef(o, d, v) -> o=student, d=data, v=qr
