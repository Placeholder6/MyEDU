...
interface OshSuApi {
    // We use ResponseBody for EVERYTHING to see raw server output without crashing
...
class DebugCookieJar : CookieJar {
    ...
    override fun saveFromResponse(url: HttpUrl, cookies: List<Cookie>) {
        // We don't care about saving for this debug session, we force our own
    }
...
object NetworkClient {
    ...
    val api: OshSuApi = Retrofit.Builder()
        ...
        .client(OkHttpClient.Builder()
            ...
            .build())
        .addConverterFactory(GsonConverterFactory.create()) // Used only for sending body
        .build()
